...
module oml_class
   ...
   type :: oml
      ...
      !> Oscillation frequency and vorticity thickness
      real(WP) :: frequency, thickness
      !> Grid dimensions
      real(WP) :: Lx,Ly,Lz
   contains
      procedure :: init          !< Initialize oml simulation
      procedure :: step          !< Advance oml simulation by one time step
      procedure :: final         !< Finalize oml simulation
...
contains
   ...
   !> Initialization of oml simulation
   subroutine init(this)
      implicit none
      class(oml), intent(inout) :: this
      ...
      create_config: block
         ...
         call param_read('Lx',this%Lx); call param_read('nx',nx); allocate(x(nx+1))
         call param_read('Ly',this%Ly); call param_read('ny',ny); allocate(y(ny+1))
         call param_read('Lz',this%Lz); call param_read('nz',nz); allocate(z(nz+1))
         ! Create simple rectilinear grid
         do i=1,nx+1; x(i)=real(i-1,WP)/real(nx,WP)*this%Lx-0.5_WP*this%Lx; end do
         do j=1,ny+1; y(j)=real(j-1,WP)/real(ny,WP)*this%Ly-0.5_WP*this%Ly; end do
         do k=1,nz+1; z(k)=real(k-1,WP)/real(nz,WP)*this%Lz-0.5_WP*this%Lz; end do
         ! General serial grid object
         grid=sgrid(coord=cartesian,no=1,x=x,y=y,z=z,xper=.true.,yper=.false.,zper=.true.,name='oml')
         ! Read in partition
      ...
      create_flow_solver: block
         ...
         ! Set density to 1.0
         this%fs%rho=1.0_WP
         ! Read in frequency and thickness
         call param_read('Frequency',this%frequency)
         call param_read('Thickness',this%thickness)
         ! Set viscosity from Reynolds number
         call param_read('Reynolds number',this%visc); this%visc=this%thickness/this%visc
      ...
      initialize_forcing: block
         ...
         do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
            this%meanU(j)=0.5_WP*tanh(0.5_WP*this%cfg%ym(j)/this%thickness)
         end do
   ...
   subroutine step(this)
      ...
      ! Define mean velocity profile at time t
      define_mean_profile: block
         integer :: i,j
         real(WP) :: A, cflf
         A=1.0_WP+0.5_WP*sin(this%frequency*this%time%t*2.0_WP*3.1415926535897932385_WP)
         do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
            this%meanU(j)=0.5_WP*A*tanh(0.5_WP*this%cfg%ym(j)/this%thickness)
         end do
         ! Update CFL
         cflf=0.0_WP
         do i=this%cfg%imin_,this%cfg%imax_
            cflf=max(cflf,abs(0.5_WP*A*this%time%dt*this%cfg%dxmi(i)))
         end do
         this%time%cfl=this%time%cfl+cflf
      end block define_mean_profile
