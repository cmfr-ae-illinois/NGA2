...
contains
   ...
   subroutine build_polygons(this,store_for_output)
      use mpi_f08, only: MPI_Wtime
      ...
      implicit none
      class(amrvof), intent(inout) :: this
      logical, intent(in), optional :: store_for_output
      logical :: update_smesh
      integer :: lvl
      real(WP) :: dx,dy,dz,t0
      integer :: i,j,k
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pCurv=>null(),pSD,pPLIC
      ...
      integer :: poly_nv
      ! Store polygons?
      if (present(store_for_output)) then
         update_smesh=store_for_output
      else
         update_smesh=this%update_smesh
      end if
      ! Start timer
      t0=MPI_Wtime()
...
module amrvof_class
   ...
   use amrsolver_class,  only: amrsolver
   use surfmesh_class,   only: surfmesh
   use amrex_amr_module, only: amrex_multifab,amrex_box,amrex_boxarray,amrex_distromap,amrex_real
   ...
   type, extends(amrsolver) :: amrvof
      ...
      ! PLIC & PIC: finest-level-only multifab
      type(amrex_multifab) :: PLIC,PLICold ! 4 components (nx, ny, nz, d)
      type(picmfab)        :: PIC,PICold
      logical :: single_plane=.true.
      ...
      real(WP) :: wt_plic=0.0_WP       !< Full build_plic
      real(WP) :: wt_plicnet=0.0_WP    !< PLICnet reconstruction loop
      real(WP) :: wt_jibben=0.0_WP     !< Jibben reconstruction loop
      real(WP) :: wt_polygon=0.0_WP    !< Polygon extraction loop
      ! Reduced timing (min/max across ranks)
      real(WP) :: wtmax_advance=0.0_WP, wtmin_advance=0.0_WP
      real(WP) :: wtmax_sl=0.0_WP,      wtmin_sl=0.0_WP
   ...
   contains
      ...
      ! VOF-PLIC methods
      procedure :: build_plic             !< Reconstruct PLIC from VF and barycenters
      procedure :: build_ppic             !< Reconstruct PLIC from VF and barycenters
      procedure :: build_plicnet          !< PLICnet reconstruction
      procedure :: build_jibben           !< Jibben interface reconstruction
      procedure :: build_polygons         !< Build polygons from PLIC planes
      procedure :: build_curved_polygons  !< Build curved polygons from PIC
      procedure :: reset_moments          !< Recompute VF/barycenters from PLIC
      procedure :: build_band             !< Build transport band around interface
      procedure :: build_subVF            !< Build sub-cell volume fractions
      ! Physics methods
   ...
   abstract interface
      subroutine vof_bc_iface(solver,lvl,time,face,bx,pVF,pCL,pCG,pPLIC)
         import :: amrvof,amrex_box,WP,pic_container
         class(amrvof), intent(inout) :: solver
         integer, intent(in) :: lvl                  !< Current level
         real(WP), intent(in) :: time
         integer, intent(in) :: face                 !< 1=xlo,2=xhi,3=ylo,4=yhi,5=zlo,6=zhi
...
contains
   ...
   subroutine finalize(this)
      use amrex_amr_module, only: amrex_multifab_destroy
      use amrpic_class,     only: picmfab_destroy
      implicit none
      class(amrvof), intent(inout) :: this
      ! Finalize VF/VFold amrdata
      call this%VF%finalize()
      ...
      call amrex_multifab_destroy(this%CG)
      call amrex_multifab_destroy(this%PLIC)
      call picmfab_destroy       (this%PIC)
      call amrex_multifab_destroy(this%CLold)
      call amrex_multifab_destroy(this%CGold)
      call amrex_multifab_destroy(this%PLICold)
      call picmfab_destroy       (this%PICold)
      call amrex_multifab_destroy(this%subVF)
      call amrex_multifab_destroy(this%curv)
      call amrex_multifab_destroy(this%SD)
      ! Finalize surface mesh
   ...
   subroutine on_remake(this,lvl,time,ba,dm)
      use amrgrid_class, only: mfab_rebuild
      use amrpic_class, only: set_to_full_or_empty
      implicit none
      class(amrvof), intent(inout) :: this
      integer, intent(in) :: lvl
      real(WP), intent(in) :: time
      ...
      if (lvl.eq.this%amr%maxlvl) then
         remake_finest: block
            ...
            call CG_new%parallel_copy(this%CG,this%amr%geom(lvl))
            call PLIC_new%parallel_copy(this%PLIC,this%amr%geom(lvl))
            call PIC_new%parallel_copy(this%PIC,this%amr%geom(lvl))
            ! Destroy old, assign new
            call this%CL%move(CL_new)
            call this%CG%move(CG_new)
            call this%PLIC%move(PLIC_new)
            call this%PIC%move(PIC_new)
            ! Rebuild curv/SD with parallel_copy of surviving data
            if (this%calculate_curv) then
               call amrex_multifab_build(curv_new,ba,dm,nc=1,ng=this%nover); call curv_new%setval(0.0_WP)
               call amrex_multifab_build(SD_new  ,ba,dm,nc=1,ng=this%nover); call SD_new  %setval(0.0_WP)
   ...
   contains
      ...
      subroutine fill_lvl(lvl)
         use amrex_amr_module, only: amrex_mfiter,amrex_mfiter_build,amrex_mfiter_destroy
         use amrpic_class, only: set_to_full_or_empty,set_to_full,set_to_empty,reflect_pic,pic_container,set_to_one_plane
         implicit none
         integer, intent(in) :: lvl
         type(amrex_mfiter) :: mfi
         type(amrex_box) :: vbx,bc_bx
         ...
         if (at_finest) then
            ...
            call this%CG%fill_boundary(this%amr%geom(lvl))
            call this%PLIC%fill_boundary(this%amr%geom(lvl))
            call this%PIC%fill_boundary(this%amr%geom(lvl)) ! This automatically shifts interfaces across periodic boundaries
         end if
         ! Step 2: periodic shift + BCs + trivial ghosts
         call amrex_mfiter_build(mfi,this%VF%mf(lvl),tiling=.false.)
         do while (mfi%next())
   ...
   !> Children can override to use their own typed callback instead
   subroutine apply_vofbc(this,lvl,time,face,bx,pVF,pCL,pCG,pPLIC)
      use amrpic_class, only: pic_container
      implicit none
      class(amrvof), intent(inout) :: this
      integer, intent(in) :: lvl
      real(WP), intent(in) :: time
   ...
   !> Build PLIC reconstruction from VF and barycenters using PLICnet
   subroutine build_ppic(this,time)
      use mpi_f08, only: MPI_Wtime
      implicit none
      class(amrvof), intent(inout) :: this
      real(WP), intent(in) :: time
      ...
      ! Build polygons and curvature from PLIC
      call this%build_polygons(store_for_output=.false.)

      ! Build Jibben-PPIC reconstruction
      call this%build_jibben(time)

      ! Build polygons and curvature from PPIC
      call this%build_curved_polygons(use_ppic_curvature=.true.)
   ...
   subroutine build_plicnet(this,time)
      ...
      use amrvof_geometry, only: get_plane_dist
      use amrex_amr_module, only: amrex_mfiter
      use amrpic_class, only: set_to_full_or_empty,set_to_one_plane,is_one_plane,pic_container
      use mpi_f08, only: MPI_Wtime
      implicit none
      class(amrvof), intent(inout) :: this
      real(WP), intent(in) :: time
   ...
   !> Jibben interface reconstruction
   subroutine build_jibben(this,time)
      use amrex_amr_module, only: amrex_mfiter
      ...
      type(amrex_mfiter) :: mfi
      type(amrex_box) :: bx,gbx
      real(WP) :: maxcurv,ivol,vol_liq

      ! Variables for Jibben fit
      integer :: ind,icenter
      ...
      dy=this%amr%dy(lvl)
      dz=this%amr%dz(lvl)
      ivol=1.0_WP/this%amr%cell_vol(lvl)
      maxcurv=1.0_WP/this%amr%min_meshsize(lvl)
      ! Compute new polygons
      call this%amr%mfiter_build(lvl,mfi,tiling=.false.)
      do while (mfi%next())
         ! Get local and grown boxes
         bx=mfi%tilebox()
         gbx=mfi%growntilebox(2)  ! Grown by 2 for 5x5x5 stencil for PPIC reconstruction
         ! Get pointer to PLIC data
         pVF=>this%VF%mf(lvl)%dataptr(mfi)
         pPLIC=>this%PLIC%dataptr(mfi)
         pPIC=>this%PIC%dataptr(mfi)
         ! ----- Step A: Compute polygons and fit PPIC using jibben -----
         do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
            ! Initialize PIC with PLIC
            call set_to_one_plane(pPIC(i,j,k,1),pPLIC(i,j,k,1:3),pPLIC(i,j,k,4))
            ! Skip cells with no interface
            if (is_full(pPLIC(i,j,k,:)).or.is_empty(pPLIC(i,j,k,:))) cycle
            ...
            do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
               ! Skip cells with no interface
               if (is_full(pPLIC(ii,jj,kk,:)).or.is_empty(pPLIC(ii,jj,kk,:))) cycle
               ! Add polygon to neighborhood
               call setNumberOfPlanes(planar_sep,1)
               call setPlane(planar_sep,0,pPLIC(ii,jj,kk,1:3),pPLIC(ii,jj,kk,4))
               ...
               if (getNumberOfVertices(polygon).ge.3) then
                  call addMember(neighborhood,polygon,1.0_WP)
            ...
            if (ind.gt.0.and.found_center) then
               ...
               call setDelta(neighborhood, 2.5_WP*(dx+dy+dz)/3.0_WP)  ! Set delta to 2.5 cell widths
               call localize(neighborhood)
               ! Perform the reconstruction
               call reconstructJibben3D(neighborhood,pPIC(i,j,k,1))
               ! Match Jibben parbolic reconstruction to volume fraction
               lo=[this%amr%xlo+real(i  ,WP)*dx,this%amr%ylo+real(j  ,WP)*dy,this%amr%zlo+real(k  ,WP)*dz]
               hi=[this%amr%xlo+real(i+1,WP)*dx,this%amr%ylo+real(j+1,WP)*dy,this%amr%zlo+real(k+1,WP)*dz]
               call construct_2pt(cell,lo,hi)
               call matchVolumeFraction(cell,pVF(i,j,k,1),pPIC(i,j,k,1))
               ! Cleanup: revert to PLIC if PPIC not successful
               call cut_rectcub_pic(lo,hi,pPIC(i,j,k,1),vol_liq)
               if (abs(vol_liq/this%amr%cell_vol(lvl)-pVF(i,j,k,1)) > 1e-13_WP) then
                  call set_to_one_plane(pPIC(i,j,k,1),pPLIC(i,j,k,1:3),pPLIC(i,j,k,4))
               end if
      ...
      call this%fill(lvl,time)
      ! End timer
      this%wt_jibben=this%wt_jibben+(MPI_Wtime()-t0)
   ...
   subroutine build_polygons(this,store_for_output)
      ...
      use amrex_amr_module, only: amrex_mfiter,amrex_multifab_destroy
      use amrgrid_class, only: mfab_rebuild
      use amrpic_class, only: is_full,is_empty,is_one_plane,get_plane,pic_container
      implicit none
      class(amrvof), intent(inout) :: this
      logical, intent(in), optional :: store_for_output
      logical :: update_smesh
      ...
      do while (mfi%next())
         ...
         do k=gbx%lo(3),gbx%hi(3); do j=gbx%lo(2),gbx%hi(2); do i=gbx%lo(1),gbx%hi(1)
            ! Skip cells with no interface
            if (is_full(pPLIC(i,j,k,:)).or.is_empty(pPLIC(i,j,k,:))) cycle         
            ! ! Skip cells with no plane or more than one plane
            ! if (.not.is_one_plane(pPLIC(i,j,k,:))) cycle          ! Unnecessary since PLIC is always 1 plane 
            ! Build hex and plane
            lo=[this%amr%xlo+real(i  ,WP)*dx,this%amr%ylo+real(j  ,WP)*dy,this%amr%zlo+real(k  ,WP)*dz]
            hi=[this%amr%xlo+real(i+1,WP)*dx,this%amr%ylo+real(j+1,WP)*dy,this%amr%zlo+real(k+1,WP)*dz]
            plane=get_plane(pPLIC(i,j,k,:))
            hex(:,1)=[hi(1),lo(2),lo(3)]
            hex(:,2)=[hi(1),hi(2),lo(3)]
            hex(:,3)=[hi(1),hi(2),hi(3)]
            hex(:,4)=[hi(1),lo(2),hi(3)]
   ...
   contains
      ...
      subroutine paraboloid_integral_fit(i,j,k,mycurv)
         use mathtools, only: normalize,cross_product,symsolve
         use amrpic_class, only: is_one_plane,get_plane,get_normal
         implicit none
         integer,  intent(in)  :: i,j,k
         real(WP), intent(out) :: mycurv
         ! Local basis vectors
         ...
         ! Local orthonormal basis from center cell PLIC normal
         if (.not.is_one_plane(pPLIC(i,j,k,:))) return
         nref=get_normal(get_plane(pPLIC(i,j,k,:)))
         select case (maxloc(abs(nref),1))
         case (1); tref=normalize([+nref(2),-nref(1),0.0_WP])
         case (2); tref=normalize([0.0_WP,+nref(3),-nref(2)])
         case (3); tref=normalize([-nref(3),0.0_WP,+nref(1)])
         ...
         do kk=k-2,k+2; do jj=j-2,j+2; do ii=i-2,i+2
            ...
            nv=poly_nv_local(ii,jj,kk); if (nv.lt.3) cycle
            ! Skip polygons with anti-aligned normal
            nloc=get_normal(get_plane(pPLIC(ii,jj,kk,:))); if (dot_product(nloc,nref).le.0.0_WP) cycle
            ! Compute neighbor centroid and transform to local frame
            ploc=sum(polygon_local(:,1:nv,ii,jj,kk),dim=2)/real(nv,WP)
            buf=(ploc-pref)*maxcurv; ploc=[dot_product(buf,tref),dot_product(buf,sref),dot_product(buf,nref)]
            buf=nloc;                nloc=[dot_product(buf,tref),dot_product(buf,sref),dot_product(buf,nref)]
   ...
   !> Build polygons and curvature from PLIC planes
   subroutine build_curved_polygons(this,use_ppic_curvature,store_for_output)
      use mpi_f08, only: MPI_Wtime
      use amrvof_geometry, only: cut_hex_polygon,poly_area
      use amrex_amr_module, only: amrex_mfiter,amrex_multifab_destroy
      use amrgrid_class, only: mfab_rebuild
      use amrpic_class, only: is_full,is_empty,is_one_plane,get_plane,pic_container,is_paraboloid
      use irl_fortran_interface
      implicit none
      class(amrvof), intent(inout) :: this
      logical, intent(in), optional :: use_ppic_curvature,store_for_output
      logical :: update_smesh, update_curv
      integer :: lvl
      real(WP) :: dx,dy,dz,t0
      integer :: i,j,k,ii,jj
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pCurv=>null(),pPLIC
      type(pic_container), dimension(:,:,:,:), contiguous, pointer :: pPIC
      real(WP), dimension(3) :: lo,hi
      real(WP), dimension(3,8) :: hex
      type(amrex_mfiter) :: mfi
      type(amrex_box) :: bx
      real(WP) :: maxcurv,ivol

      ! For surface output
      integer, dimension(6) :: connectivity
      real(WP), dimension(4,6) :: vertices 
      real(WP), dimension(3,6) :: poly_verts
      integer :: poly_nv,nbt
      type(MixedPolygonBezierSurface_type) :: mixed_surface
      type(RectCub_type) :: cell

      ! Storage for a our IRL objects
      call new(cell)
      call new(mixed_surface)
      ...
      ! Update curvature?
      if (present(use_ppic_curvature)) then
         update_curv=use_ppic_curvature
      else
         update_curv=.true.
      end if
      ! Start timer
      ...
      do while (mfi%next())
         ! Get local and grown boxes
         bx=mfi%tilebox()
         ! Get pointer to PLIC and PIC data
         pPLIC=>this%PLIC%dataptr(mfi)
         pPIC=>this%PIC%dataptr(mfi)
         ! ----- Step A: Update curvature (valid cells with PPIC) -----
         if (this%calculate_curv) then
            ! Get pointer to curvature data
            pCurv=>this%curv%dataptr(mfi)
            if (update_curv) then
               do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
                  if (is_paraboloid(pPIC(i,j,k,1))) then
                     pCurv(i,j,k,1)=getMeanCurvature(pPIC(i,j,k,1))
                     pCurv(i,j,k,1)=max(min(pCurv(i,j,k,1),maxcurv),-maxcurv)
                  end if
               end do; end do; end do
            end if
         end if
         ! ----- Step B: Append to smesh (valid cells only) -----
         if (update_smesh) then
            do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
               ! Skip cells with no interface
               ...
               lo=[this%amr%xlo+real(i  ,WP)*dx,this%amr%ylo+real(j  ,WP)*dy,this%amr%zlo+real(k  ,WP)*dz]
               hi=[this%amr%xlo+real(i+1,WP)*dx,this%amr%ylo+real(j+1,WP)*dy,this%amr%zlo+real(k+1,WP)*dz]
               ! CASE: Polygonal surface
               if (is_one_plane(pPIC(i,j,k,1))) then
                  ...
                  call cut_hex_polygon(hex,get_plane(pPIC(i,j,k,1)),poly_nv,poly_verts)
                  if (this%calculate_curv) then
                     call this%smesh%add_polygon(nv=poly_nv,verts=poly_verts,vardata=[pCurv(i,j,k,1)])
                  else
                     call this%smesh%add_polygon(nv=poly_nv,verts=poly_verts)
                  end if
               else ! CASE: Curved surface
                  call construct_2pt(cell,lo,hi)
                  call zeroMixedSurface(mixed_surface)
                  call getSurface(cell,pPIC(i,j,k,1),mixed_surface)
                  nbt=getNumberOfTriangles(mixed_surface)
                  do ii=0,nbt-1
                     connectivity=getTri(mixed_surface,ii)
                     do jj=1,6
                        vertices(:,jj)=getPt(mixed_surface,connectivity(jj))
                     end do
                     if (this%calculate_curv) then
                        call this%smesh%add_bezier_tri(verts=vertices,vardata=[pCurv(i,j,k,1)])
                     else
                        call this%smesh%add_bezier_tri(verts=vertices)
                     end if
                  end do
               end if
            end do; end do; end do  
      ...
      ! End timer
      this%wt_polygon=this%wt_polygon+(MPI_Wtime()-t0)
   end subroutine build_curved_polygons

   !> Reset VF and barycenters from PLIC plane to ensure consistency
   !> Computes in valid + ghost cells from PLIC (which is already filled)
   subroutine reset_moments(this)
      use amrpic_class, only: is_full,is_empty,cut_rectcub_pic,pic_container
      use amrvof_geometry, only: cut_hex_vol
      use amrex_amr_module, only: amrex_mfiter
      use irl_fortran_interface, only: isTypeDefined
      implicit none
      class(amrvof), intent(inout) :: this
      integer :: lvl,i,j,k
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pVF,pCL,pCG
      type(pic_container), dimension(:,:,:,:), contiguous, pointer :: pPIC
      real(WP), dimension(3) :: ptlo, pthi
      real(WP) :: vol_liq,vol_gas,cell_vol,dx,dy,dz
      real(WP), dimension(3) :: bary_liq,bary_gas,cell_center
      type(amrex_mfiter) :: mfi
      type(amrex_box) :: bx
      ...
      do while (mfi%next())
         ...
         do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
            ...
            ! Lower and upper bounds of rectangular cuboid
            ptlo=[this%amr%xlo+real(i  ,WP)*dx,this%amr%ylo+real(j  ,WP)*dy,this%amr%zlo+real(k  ,WP)*dz]
            pthi=[this%amr%xlo+real(i+1,WP)*dx,this%amr%ylo+real(j+1,WP)*dy,this%amr%zlo+real(k+1,WP)*dz]
            ! Cut rectangular cuboid by interface
   ...
   subroutine build_subVF(this)
      ...
      class(amrvof), intent(inout) :: this
      integer :: lvl,i,j,k
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pSubVF
      type(pic_container), dimension(:,:,:,:), contiguous, pointer :: pPIC
      real(WP), dimension(3) :: ptlo, pthi
      real(WP) :: vol_liq,vol_gas,half_vol,dx,dy,dz,xlo,xhi,ylo,yhi,zlo,zhi,xcc,ycc,zcc
      real(WP), dimension(3) :: bary_liq,bary_gas
      ...
      do while (mfi%next())
         ...
         do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
            ...
            ylo=this%amr%ylo+real(j,WP)*dy; yhi=this%amr%ylo+real(j+1,WP)*dy; ycc=0.5_WP*(ylo+yhi)
            zlo=this%amr%zlo+real(k,WP)*dz; zhi=this%amr%zlo+real(k+1,WP)*dz; zcc=0.5_WP*(zlo+zhi)
            ! Check that interface type is well defined before cutting
            if (.not.isTypeDefined(pPIC(i,j,k,1))) then
               print *, "Interface at ", i,j,k, " has no type"
            end if      
            ! SubVF(1)=x-lo half 
   ...
   subroutine advance_vof(this,U,V,W,dt,time)
      ...
      ! Shared variables for internal functions
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pU,pV,pW ! Velocity used for project
      type(pic_container), dimension(:,:,:,:), contiguous, pointer :: pPICold ! PIC old used in tet2flux_plic
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pVFold   ! VFold used in tet2flux_plic
      ! Start full routine timer
      t0=MPI_Wtime()
      ...
      t1=MPI_Wtime() ! Start SL timer
      compute_fluxes: block
         use amrpic_class, only: is_full,is_empty
         use amrvof_geometry, only: tet_sign,tet_map,tet_vol,correct_flux_poly,flux_poly_moments
         integer :: i,j,k,n,nn
         real(WP), dimension(3,9) :: face
         real(WP), dimension(3,4) :: tet
         ...
         do while (mfi%next())
            ...
            do k=fbx%lo(3),fbx%hi(3); do j=fbx%lo(2),fbx%hi(2); do i=fbx%lo(1),fbx%hi(1)
               ...
               bblo=[minval(fijk(1,:)),minval(fijk(2,:)),minval(fijk(3,:))]
               bbhi=[maxval(fijk(1,:)),maxval(fijk(2,:)),maxval(fijk(3,:))]
               bb_pure_liq=is_full (pPICold(bblo(1):bbhi(1),bblo(2):bbhi(2),bblo(3):bbhi(3),:))
               bb_pure_gas=is_empty(pPICold(bblo(1):bbhi(1),bblo(2):bbhi(2),bblo(3):bbhi(3),:))
               ! Compute volume flux
               pVx(i,j,k,:)=0.0_WP
               if (bb_pure_liq.or.bb_pure_gas) then
                  ! Lightweight path
   ...
   contains
      ...
      function tet2flux_plic(mytet,i0,j0,k0) result(myflux)
         ...
         ! Cut tet by interface
         if (.not.isTypeDefined(pPICold(i0,j0,k0,1))) then
   ...
   subroutine register_checkpoint(this,io)
      ...
      call io%add_mfab   (this%CG,'CG',this%amr%maxlvl)
      call io%add_mfab   (this%PLIC,'PLIC',this%amr%maxlvl) 
      call io%add_picmfab(this%PIC,'PIC',this%amr%maxlvl) 
   end subroutine register_checkpoint
   ...
   subroutine restore_checkpoint(this,io,dirname,time)
      ...
      call io%read_mfab   (dirname,this%CG,'CG',this%amr%maxlvl)
      call io%read_mfab   (dirname,this%PLIC,'PLIC',this%amr%maxlvl) 
      call io%read_picmfab(dirname,this%PIC,'PIC',this%amr%maxlvl) 
      ! Fill ghosts
      call this%fill(time=time)
      ! Rebuild polygons from restored PLIC
      if (this%single_plane) then
         call this%build_polygons(store_for_output=.true.)
      else
         call this%build_polygons(store_for_output=.false.)
         call this%build_curved_polygons(store_for_output=.true.)
      end if
   end subroutine restore_checkpoint
